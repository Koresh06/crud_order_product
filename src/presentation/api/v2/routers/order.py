...
from src.presentation.api.v2.schemas.requests.order import CreateOrderRequest, UpdateOrderRequest
from src.presentation.api.v2.schemas.responses.order import OrderResponse, OrdersListResponse
...
async def create_order(
    ...
    Принимает объект `CreateOrderRequest` и возвращает созданный заказ.
    """
...
async def update_order(
    ...
    payload: Annotated[
        UpdateOrderRequest,
        Body(..., description="Данные для обновления заказа"),
    ],
    user_case: Annotated[
        UseCaseOneEntity,
        Depends(Provide[Container.update_order_uc]),
    ],
) -> OrderResponse:
    """
    Обновление существующего заказа по его ID.

    Принимает объект `UpdateOrderRequest` и возвращает обновлённый заказ.
    """
    order_dto = await user_case.execute(OrderId(order_id), payload.to_dto())
    return OrderResponse.from_dto(order_dto)
...
async def delete_order(
    ...
    user_case: Annotated[
        UseCaseNoReturn,
        Depends(Provide[Container.delete_order_uc]),
    ],
) -> None:
    """
    Удаление заказа по его уникальному идентификатору.
    """
    await user_case.execute(OrderId(order_id))
